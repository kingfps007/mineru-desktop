...
MinerU CLI 用命令行替代了不稳定的 Electron 桌面应用——从一个 71MB 的 GUI（启动 10 秒、三层进程通信容易崩溃、每 10 篇就重新加载 AI 模型）精简为 7.2MB 的终端工具（即时启动、单进程直接调用、一次任务只加载一次模型）。
...
| 🌐 中英双语 | 全界面实时切换 |
| ⚡ 单次加载 | 所有 PDF 临时复制到一个文件夹，MinerU 只调用一次——不再每批重新加载模型 |
