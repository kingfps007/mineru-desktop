...
# v3.2.9 回退到 3.1.2 策略：BATCH_SIZE=10 已被用户在 300+ 篇验证（只崩 1 次），
...
def sanitize_name(name):
    ...
    v3.2.9 修复：中文文件名+尾部空格导致 mineru 内部 os.makedirs 失败。
    """
    name = name.strip()
    for ch in '<>:"/\\|?*':
...
def cleanup_memory():
    """释放显存 + 清理残留进程（v3.2.9 回退到 3.1.2 风格）
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "version": "3.2.9"}

@app.get("/api/quick")
async def quick_status():
    return {
        "status": "ok", "version": "3.2.9",
...
async def cancel_parse():
    """v3.2.9 回退到 3.1.2 优雅风格：terminate → wait 2s → kill
