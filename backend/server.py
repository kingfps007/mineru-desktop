...
# ── API opener（按 config 动态选直连 / 代理）──
# v3.3.9 改动：矿 API（mineru.net）按调用方 IP 区域返回 code=-60023 "regional regulations"。
# 用户在「解析页 → 精准 API 配置」可勾选「启用代理」+ 填代理地址（如境外代理）绕开区域限制。
# 默认：强制直连（ProxyHandler({})），绕开系统被劫持的代理。
# 勾选后：按 url 走代理（适合境外代理 IP）。

_API_OPENER_CACHE = {"key": None, "opener": None}
_API_OPENER_LOCK = threading.Lock()

def _get_api_opener():
    """根据 config 返回合适的 opener：
    - api_use_proxy=True + api_proxy_url 非空 → 用该 URL 作 http/https 代理
    - 否则 → 强制直连（ProxyHandler({})，绕开系统被劫持代理）
    带 cache：相同配置复用同一 opener，避免每次新建 handler chain。
    """
    cfg = load_config()
    use_proxy = bool(cfg.get("api_use_proxy", False))
    proxy_url = (cfg.get("api_proxy_url") or "").strip() or "http://127.0.0.1:7897"
    key = (use_proxy, proxy_url)
    with _API_OPENER_LOCK:
        if _API_OPENER_CACHE["key"] == key and _API_OPENER_CACHE["opener"] is not None:
            return _API_OPENER_CACHE["opener"]
        ...
        _API_OPENER_CACHE["key"] = key
        _API_OPENER_CACHE["opener"] = opener
        return opener

def _api_urlopen(req, timeout=None):
    """调矿 API（按 config 决定直连 / 代理）。封装 _get_api_opener() 的 open 调用。"""
    return _get_api_opener().open(req, timeout=timeout)
...
def cleanup_memory():
    ...
    1. **不要 `taskkill /F /T /IM mineru.exe`** —— 这会**杀当前正在解析的 mineru.exe**，
       引发"清理内存后程序退出"。3.1.2 的精准扫描（只杀 `mineru-api` HTTP server 残留）
       被用户用 300+ 篇验证过：稳定不误杀。
    2. **杀 `mineru-api` 字符串的进程** —— 这是 mineru HTTP server 启动时的映像名，
       CLI 解析时**不会出现**这个字符串。
    3. **保留 spawn torch 子进程** —— 3.1.2 就有这个逻辑（`import torch; empty_cache`）。
       v3.2.5 因为 hang 风险去掉是过度保守。
    """
    released = []
    killed = 0

    # 1. 精准扫描 tasklist：只杀 mineru-api 残留（HTTP server 残留）
    #    关键：不能杀 `mineru` 字符串的所有 .exe，否则会杀解析中的 mineru.exe
    try:
        r = subprocess.run(['tasklist', '/fo', 'csv', '/nh'],
                           capture_output=True, text=True, timeout=5)
        for line in r.stdout.splitlines():
            low = line.lower()
            # 3.1.2 验证过的条件：mineru-api 字符串 OR uvicorn 但不含 mineru
        ...
        sync_log("⚠ 扫描 tasklist 超时（>5s），跳过")
    except Exception as e:
        sync_log(f"⚠ 扫描进程时出错: {e}")
    if killed:
        released.append(f"终止 {killed} 个残留进程")
    ...
    after = _get_mem_mb()
    if before - after > 10:
        released.append(f"释放内存 {before - after:.0f} MB")

    # 3. GPU 显存释放（3.1.2 风格的 spawn torch 子进程）
    #    触发条件：环境已就绪（env_ready）。否则跳过（避免 hang）
    try:
        env = cached_detect_mineru_env()
        python_exe = env["python"] if env and env.get("python") else None
        if python_exe:
            subprocess.run(
                [python_exe, "-c",
...
class ConfigData(BaseModel):
    ...
    batch_size: int = 10
    # v3.3.9 矿 API 代理设置：默认直连，勾选后走代理（适合境外代理 IP 绕开区域限制）
...
async def validate_api_token(data: dict):
    ...
    try:
        ...
        resp = _api_urlopen(req, timeout=10)
...
def _cloud_parse_core(items, output_dir, model_version, api_token, lang):
    ...
    for idx, it in enumerate(items):
        ...
        try:
            ...
            with _api_urlopen(req, timeout=30) as resp:
            ...
            _api_urlopen(upload_req, timeout=600).read()
            sync_log(f"  ✓ 上传成功")
            ...
            while polled < max_wait:
                ...
                with _api_urlopen(qreq, timeout=30) as qresp:
            ...
            with _api_urlopen(zip_url, timeout=600) as zresp:
...
def _agent_parse_core(items, output_dir, lang):
    ...
    for idx, it in enumerate(items):
        ...
        try:
            ...
            with _api_urlopen(req, timeout=120) as resp:
                rdata = json.loads(resp.read().decode("utf-8"))
            ...
            if md_url:
                ...
                with _api_urlopen(md_url, timeout=300) as mresp:
                    md_bytes = mresp.read()
                dst = Path(output_dir) / it["name"]
                dst.mkdir(parents=True, exist_ok=True)
...
def _batch_parse_core(items, env, output_dir, backend, lang):
    ...
    for batch_idx in range(total_batches):
        ...
        for it in batch:
            try:
                safe_name = sanitize_name(it["name"])
                shutil.copy2(it["pdf_path"], batch_dir / f"{safe_name}.pdf")
                it["_safe_name"] = safe_name
            except Exception as e:
        ...
        while retry < MAX_RETRIES and not batch_success:
            ...
            try:
                ...
                if proc.returncode == 0:
                    for it in batch:
                        safe_name = it.get("_safe_name", it["name"])
                        src = batch_out / safe_name
                        dst = Path(output_dir) / it["name"]
                        if src.exists():
...
async def start_parse(req: ParseRequest):
    ...
    items = [{"name": sanitize_name(r["citekey"]), "pdf_path": r["pdf_path"]} for r in selected]
...
async def cancel_parse():
    ...
    if stop: stop.set()
    if proc:
        try: proc.terminate()    # 1. 优雅终止（Ctrl+C 风格）
        except: pass
        try: proc.wait(timeout=2)  # 2. 等待 2s 让 worker 清理
        except: pass
        try: proc.kill()         # 3. 兜底强杀
        except: pass
